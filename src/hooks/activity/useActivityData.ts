...
const generateMockActivities = (): ActivityItem[] => {
  ...
  const types = ['page_load', 'crawl_started', 'pdf_download', 'job_added', 'settings_changed'];
  const actions = ['page_load', 'crawl_started', 'single_pdf_download', 'bulk_download', 'job_added'];
...
const calculateStats = (activities: ActivityItem[]): ActivityStats => {
  ...
  const topActivityType = Object.entries(activityBreakdown).reduce((max, [type, count]) =>
    count > (activityBreakdown[max] || 0) ? type : max, ''
  );

  // Average per day (based on date range)
  const dateRange = Math.max(1, Math.ceil((Date.now() - activities[0]?.timestamp?.getTime() || Date.now()) / (24 * 60 * 60 * 1000)));
...
export const useActivityData = (filters: ActivityFilters): UseActivityDataResult => {

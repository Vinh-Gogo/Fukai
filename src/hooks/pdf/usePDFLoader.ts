...
export const usePDFLoader = (initialUrl?: string): UsePDFLoaderResult => {
  ...
  const categorizeError = useCallback(
    (error: unknown, url?: string): PDFLoaderError => {
      const originalError =
      ...
      if (
        ...
        originalError.message.includes("CORS")
      ) {
        return {
          type: "network",
          ...
          retryable: true,
          userMessage:
            "Unable to download the PDF file. Please check your internet connection and try again.",
          url,
        };
      }
      ...
      if (
        ...
        return {
          ...
          retryable: true,
          userMessage:
            "The PDF file took too long to load. The file might be large or the server is slow.",
          url,
        };
      }
      ...
      if (
        ...
        originalError.message.includes("access") ||
      ...
      if (
        originalError.message.includes("corrupted") ||
        ...
        return {
          ...
          retryable: false,
          userMessage:
            "This PDF file appears to be corrupted or invalid. Please try a different file.",
          url,
        };
      }
      ...
      // Unsupported format
      if (
        originalError.message.includes("unsupported") ||
        ...
        return {
          ...
          retryable: false,
          userMessage:
            "This file format is not supported. Please upload a valid PDF file.",
          url,
        };
      }
      ...
      return {
        ...
        retryable: true,
        userMessage:
          "An unexpected error occurred while loading the PDF. Please try again.",
        url,
      };
    },
    [],
  ...
  const loadPDF = useCallback(
    async (url: string, retryCount = 0) => {
      ...
      if (!isSupported) {
        const pdfError = categorizeError(
          new Error("PDF loading not supported"),
        );
      ...
      try {
        ...
        if (pdfError.retryable && retryCount < 2) {
          console.log(
            `Auto-retrying PDF load (attempt ${retryCount + 1}/3)...`,
          );
          setTimeout(
            () => {

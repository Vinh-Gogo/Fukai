...
interface UseThemeAwareStylesReturn extends ThemeAwareStyles {
  getItemBackground: (isActive: boolean, isHovered: boolean, gradient: string) => string;
...
export function useThemeAwareStyles(): UseThemeAwareStylesReturn {
  ...
  const styles = useMemo((): ThemeAwareStyles => ({
    mounted,
    theme,
    sidebarBackground: mounted && theme === "dark"
      ? `linear-gradient(160deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)`
      : `linear-gradient(160deg, #f8fafc 0%, #e2e8f0 50%, #f1f5f9 100%)`,
    sidebarBorder: mounted && theme === "dark"
    ...
    glassOverlay: mounted && theme === "dark"
  ...
  const getItemTextColor = useMemo(() =>
    (isActive: boolean, isHovered: boolean): string => {
      ...
      return mounted && theme === "dark" ? "text-blue-100 hover:text-white" : "text-gray-700 hover:text-gray-900";
    }, [mounted, theme]);

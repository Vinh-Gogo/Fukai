...
export const useCrawlStats = (jobs: CrawlJob[]): UseCrawlStatsResult => {
  const stats = useMemo((): CrawlStats => {
    ...
    const jobsWithSuccessRate = jobs.filter(job => job.successRate !== undefined);
    const averageSuccessRate = jobsWithSuccessRate.length > 0
      ? Math.round(jobsWithSuccessRate.reduce((sum, job) => sum + (job.successRate || 0), 0) / jobsWithSuccessRate.length)
      : 0;

 ...
 */
export function useCrawlJobForm(
  ...
  const validateInputUrl = useCallback(
    (inputUrl: string): { isValid: boolean; error?: string } => {
      if (!inputUrl.trim()) {
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to add crawl job";

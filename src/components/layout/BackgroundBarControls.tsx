...
export function BackgroundBarControls({
  ...
  const [variant, setVariant] = useState<'gradient' | 'solid' | 'text'>(currentVariant);
  const [height, setHeight] = useState<'sm' | 'md' | 'lg'>(currentHeight);

  const handleVariantChange = useCallback((newVariant: 'gradient' | 'solid' | 'text') => {
    setVariant(newVariant);
    onVariantChange?.(newVariant);
  }, [onVariantChange]);

  const handleHeightChange = useCallback((newHeight: 'sm' | 'md' | 'lg') => {
    setHeight(newHeight);

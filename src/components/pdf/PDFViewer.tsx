        ...
        <div className="h-full flex items-center justify-center bg-gray-100 p-8">
          <div className="text-center max-w-md mx-auto">
            ...
            <p className="text-gray-600 mb-4">
              Something went wrong while rendering the PDF. This might be due to
              a corrupted file or browser compatibility issue.
            </p>
...
export const PDFViewer = memo(({ file, onClose }: PDFViewerProps) => {
  ...
  const [searchResults, setSearchResults] = useState<
    Array<{ page: number; text: string; index: number }>
  >([]);
  ...
  useEffect(() => {
    ...
    visiblePages.forEach(async (pageNumber) => {
      ...
      try {
        const scale =
          calculateScale(canvas.clientWidth, 600, zoomLevel) * devicePixelRatio;
    ...
    async (pageNumber: number, canvas: HTMLCanvasElement) => {
      ...
      try {
        ...
        if (!(err instanceof Error) || !err.message.includes("cancelled")) {
          console.error(
            `Error rendering thumbnail for page ${pageNumber}:`,
            err,
          );
        }
      }
    ...
    >
      <div className="h-full flex flex-col bg-white rounded-xl overflow-hidden shadow-xl">
        ...
        >
          ...
          <div
            style={{
              height: `${totalPages * 1100 * zoomLevel}px`,
              position: "relative",
            }}
          >
            {visiblePages.map((page) => {
              ...
              return (
                ...
                >
                                    <div className="h-full w-full bg-amber-50 border border-amber-200 rounded-lg shadow-sm overflow-hidden">
                    <div
                      className="p-4 border-b border-amber-200 bg-amber-100 flex justify-between items-center"
                      style={{ height: "40px" }}
                    >
          ...
          {/* Thumbnail sidebar */}
          <div className="hidden md:block absolute right-4 top-1/2 transform -translate-y-1/2 bg-amber-50/90 backdrop-blur-sm rounded-xl shadow-lg p-2 border border-amber-200 max-h-[80%] overflow-y-auto">
            <div className="space-y-1 pr-1">
              <div className="px-2 py-1 text-xs font-medium text-amber-900 border-b border-amber-200 mb-1">
                Pages ({totalPages})
              </div>
              {Array.from({ length: totalPages }).map((_, index) => {
                ...
                return (
                  ...
                  >
                    <div className="bg-amber-100 rounded w-12 h-16 mb-1 flex items-center justify-center overflow-hidden">
                      <canvas
                        ref={(el) => {
                          if (el && pdfDoc) {
                    ...
                    <span className="text-[10px] font-medium text-amber-900">{page}</span>
                  </button>
                );
              })}
            </div>

...
export const useIsErrorBoundaryTriggered = () => useErrorStore((state) => state.isErrorBoundaryTriggered)

export const useErrorById = (id: string) => useErrorStore((state) =>
  state.errors.find((error) => error.id === id)
)

export const useHasErrors = () => useErrorStore((state) =>
  state.errors.length > 0 || state.globalError !== null
)

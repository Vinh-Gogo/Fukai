...
export function validateFileDelete(searchParams: URLSearchParams) {
  ...
  if (!filename) {
    return {
      success: false,
      errors: [
        { field: "filename", message: "Filename parameter is required" },
      ],
    };
  }
...
export function getFileTypeCategory(mimeType: string): string {
  if ((ALLOWED_FILE_TYPES.PDF as readonly string[]).includes(mimeType))
    return "pdf";
  if ((ALLOWED_FILE_TYPES.IMAGES as readonly string[]).includes(mimeType))
    return "image";
  if ((ALLOWED_FILE_TYPES.DOCUMENTS as readonly string[]).includes(mimeType))
    return "document";
  return "unknown";
}

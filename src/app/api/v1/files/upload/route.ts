...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { file } = (validation as { success: true; data: { file: File } }).data;

    // Additional file validation
    if (file.size > FILE_CONFIG.MAX_SIZE) {
      return createFileTooLargeResponse(FILE_CONFIG.MAX_SIZE_FORMATTED, requestId);
    }

    if (!FILE_CONFIG.ALLOWED_TYPES.includes(file.type as typeof FILE_CONFIG.ALLOWED_TYPES[number])) {
      return createInvalidFileTypeResponse([...FILE_CONFIG.ALLOWED_TYPES], requestId);
    }
    ...
    try {
      ...
      return createStorageErrorResponse('create directory', undefined, requestId);
    }
    ...
    try {
      ...
      return createStorageErrorResponse('write file', error instanceof Error ? error.message : undefined, requestId);
    }
    ...
    console.log(`[${requestId}] File uploaded successfully: ${uniqueFilename} (${formatFileSize(file.size)})`);
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('quota') || error.message.includes('space')) {
        return createStorageErrorResponse('upload', 'Insufficient storage space', requestId);
      }
    }

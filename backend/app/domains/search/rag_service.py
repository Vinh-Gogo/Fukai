...
class RAGService:
    ...
    async def query_documents(
        ...
        """
        Query documents using RAG approach with caching
        ...
        if self.cache_service:
            cache_key = f"{query}:{user_id}:{limit}:{score_threshold}:{include_metadata}"
            cached_result = await self.cache_service.get_search_results(cache_key)
            if cached_result:
                logger.debug(f"Returning cached RAG result for query: {query[:50]}...")
                # Convert cached data back to RAGResult
                return RAGResult(
                    query=query,
                    relevant_chunks=cached_result,
                    total_chunks_found=len(cached_result),
                    processing_time=0.001,  # Minimal processing time for cache hit
        ...
        try:
            ...
            if not query_embedding:
                ...
                if self.cache_service:
                    await self.cache_service.cache_search_results(cache_key, [], ttl=300)
                return result
            ...
            if not search_result["success"]:
                ...
                if self.cache_service:
                    await self.cache_service.cache_search_results(cache_key, [], ttl=60)  # Shorter TTL for errors
                return result
            ...
            for result_item in search_result["results"]:
                ...
                if include_metadata:
                    chunk_data["metadata"] = {
                        "qdrant_id": result_item["id"],
                        "user_id": payload.get("user_id"),
                        "search_score": result_item["score"]
                    }
            ...
            if self.cache_service and relevant_chunks:
                await self.cache_service.cache_search_results(cache_key, relevant_chunks, ttl=600)  # 10 minutes
            ...
            if self.cache_service:
                await self.cache_service.cache_search_results(cache_key, [], ttl=60)
